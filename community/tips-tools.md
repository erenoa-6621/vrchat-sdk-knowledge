...
### NDMF ExMenu Organizer
- **作者**: ゆいちゃまーと
- **入手**: https://booth.pm/ja/items/7841621
- **価格**: ¥500（機能差なし）
- **用途**: アバターのエクスプレッションメニューをドラッグ＆ドロップで直感的に整理する非破壊ツール
- **主要機能**:
  - ドラッグで項目を並べ替え・フォルダ化できる専用UI
  - 既存メニュー設定をワンボタンでインポート
  - 削除したアイテムに対応する**未使用パラメーターを自動削除**してアバターメモリを節約
  - 日本語・英語バイリンガル対応
- **必要環境**: VRChat SDK 3.0（Avatars）+ Modular Avatar（NDMFベース）
- **出典**: https://booth.pm/ja/items/7841621

### NDMF RemoveFX
- **作者**: ゆいちゃまーと
- **入手**: https://booth.pm/ja/items/7861390
- **価格**: ¥300
- **用途**: 不要な FX レイヤーをチェックボックスで選択して非破壊的に削除するツール
- **主要機能**:
  - インスペクターのチェックボックスで削除したいレイヤーを選択してアップロード
  - 削除したレイヤーでしか使われていない**パラメーターを自動削除**
  - NDMF ベースのため元アバターデータを改変しない
- **活用場面**: 購入アバターの不要なデフォルトギミック FX（例: 配布元衣装の揺れ制御レイヤー）を衣装改変後に安全に除去する
- **必要環境**: NDMF（Non-Destructive Modular Framework）
...
### かろやかテクスチャ
- **入手**: https://luminance1116.booth.pm/items/8480183
- **価格**: 無料
- **用途**: VRChatアバターのテクスチャを1クリックで約40%削減する軽量化ツール（2026年6月配布開始）
- **主要機能**:
  - ワンクリックで最適なテクスチャサイズに自動圧縮（容量を約40%カット）
  - 顔・瞳テクスチャは自動的に保護対象になり劣化しない
  - Quest/Android 対応
  - 原本復元機能付き（後から元に戻せる）
- **NDMF対応**: NDMFパイプラインに組み込まれており、Modular Avatar と連携可能
- **LAC との使い分け**: LAC: Avatar Compressor は複雑度解析による精細なプリセット制御が可能。「かろやかテクスチャ」は手軽さ重視のワンクリック型
...
### Editor Log でアップロードファイルの容量割合を確認する

最適化の優先順位を決める前に、まず何が容量を占めているかを確認する手法。

**手順:**
1. Unity で `Console` タブを開く
2. 右上の三点メニュー（「⋮」または「≡」）をクリック
3. 「**Open Editor Log**」を選択してログファイルを開く
4. ログ内で `prefab.unity3d` を検索する（Ctrl+F）
5. ヒットした箇所に各アセット（テクスチャ・メッシュ・アニメーション等）の容量割合が一覧表示される

**活用法:** 容量トップのアセットから優先的に最適化することで効率的に容量削減できる。例えば「テクスチャが80%」なら lilAvatarUtils やかろやかテクスチャによるテクスチャ圧縮を先に実施する。
