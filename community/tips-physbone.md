...
## PhysBoneで誰でも掴めるアイテムを作る

キーホルダーやアクセサリーなど「掴んで動かせるアイテム」を実装する際の具体的な手順。

### オブジェクト構造（必須）

```
PhysBoneRoot（PhysBoneコンポーネントを持つ）
└── Model（アイテムの見た目）← 位置Y: 0.1 程度ずらす
```

**重要:** 親子間に必ず少し距離を持たせる（Y=0.1 程度）。距離がゼロだと掴んでいる見た目が正しく機能しない。

### 4ステップ設定

**①Forces タブ**
```
Pull:    0.35〜1.0（小さいほどゆっくり元の位置に戻る）
Spring:  0          ← 揺れ不要なので0
Immobile: 1         ← 動き出しをしっかり固定
```

**②Collision タブ**
```
Radius: アイテムの大きさに合わせた値（カーブで球形に調整）
Allow Collision: False（掴みだけ使う場合は衝突判定不要）
```

**③Stretch & Squish タブ**
```
Max Stretch: 2〜5以上（親子距離が短いほど大きい値が必要）
Stretch Motion: 0（掴むことが主目的なら伸び感は不要）
```
> Max Stretch は「倍率」（メートルではない）。距離0.1mで値2なら最大0.2m伸びる。掴んでいる様子が見えない場合はここを大きくする。

**④Grab & Pose タブ**
```
Allow Grabbing: True    ← 必須
Snap To Hand:   True    ← ON推奨（手にぴたっとくっつく）
Allow Posing:   好みで
```

### 掴めない場合のチェックリスト

1. `Allow Grabbing` が True になっているか
2. `Radius`（Collision タブ）が 0 より大きいか
3. 親と子の距離が 0 になっていないか
4. `Max Stretch` が小さすぎないか（距離が短い場合は特に）
5. 相手のVRChat設定でインタラクト許可が制限されていないか
